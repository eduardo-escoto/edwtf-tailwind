...
function compileOutputHTML(data) {
  const pipeline = unified()
    ...
    .use(rehypePrism)
    .use(rehypeStringify)
  return pipeline.processSync(`<div style="overflow-x: auto;">${data.join('').trim()}</div>`).value
}
...
function compileOutputPlainText(data) {
  ...
  return pipeline.processSync(
    `<pre><samp>${data.join('').trim().replace('<', '&#60;').replace('>', '&#62;')}</samp></pre>`
  ).value
}
...
function compileOutputStream({ text }) {
  ...
  return pipeline.processSync(`<pre><samp>${text.join('').trim()}</samp></pre>`).value
}
...
function compileOutputExecuteResult({ data }) {
  const compiledData = Object.keys(data)
    ...
    .map((key) => {
      const trimmedData = data[key].map((el) => (el ? el.trim() : ''))
      return dataCompilers[key](trimmedData)
    })
...
function compileCode({ source, language }) {
  const pipeline = unified()
    ...
    .use(rehypePrism)
    .use(rehypeStringify)
  const sourceToCompile = source ? source.join('').trim() : ''
  return pipeline.processSync(
...
export default async function compileNotebook({ cells, metadata }) {
  ...
  const allHtml = compiled_cells.join('').trim()
